...
为了利用帧内预测残差特殊的空间相关性，进一步提高视频帧内编码的效率，提出了一种通用的基于残差中值边缘检测的无损帧内编码算法。算法首先对帧内预测残差逐点进行边缘检测，通过分析临近点的数值特征，使用中值边缘检测算法得到当前点的预测值；然后，对比预测值与原始残差值，得到新的预测残差；最后，为了确保编码新的预测残差能够提高压缩率，依照当前编码单元的能量是否降低来快速判断是否使用新的残差进行熵编码。
