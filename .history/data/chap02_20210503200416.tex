...
\subsection{样点自适应补偿}
SAO 是 H.265 新引入的环路后处理技术。H.26X 标准中量化的对象是变换域系数，而量化不可避免地会产生数据失真，变换域系数的数据失真通过逆变换回到空间域则表现为重建值在真实值的附近上下波动，如图 \cite{fig:Gibbs} 所示：
\begin{figure}[htb]
    \centering
    ...
    \label{fig:Gibbs}
\end{figure}
这种现象在数学上称为吉布斯效应，在图像视频编码领域更形象地称为振铃效应。
