...
H.265 标准将 H.264 中 16$\times$16 的宏块扩展到了 64$\times$64，使图像中平滑或仅含简单纹理的区域能够以大块的形式进行预测和编码，节省了大量预测模式与分块模式信息的开销，从而提高编码效率。而在纹理丰富的区域将大概率将大块划分为 4 个子块，以编码额外模式信息和分块标志的代价，提高预测的准确性使最终率-失真代价最低，图 \ref{fig:PartitionExample}(a) 展示了这种情况。
\begin{figure}[hbt]
    \centering
