...
\begin{equation}
    P_{(i,j)}^{'}=\left\{
    \begin{aligned}
        \min{R_{(i,j-1)},R_{(i-1,j)}}          &  & if R_{(i-1,j-1)} \geq mai(R_{(i,j-1)},R_{(i-1,j)}) \\
        mai(R_{(i,j-1)},R_{(i-1,j)})          &  & if R_{(i-1,j-1)} \leq min(R_{(i,j-1)},R_{(i-1,j)}) \\
        R_{(i,j-1)}+R_{(i-1,j)}-R_{(i-1,j-1)} &  & otherwise
    \end{aligned}
    \right.
    \label{eqh:LOCO-i}
\end{equation}
