...
最后，帧内预测过程还包含缺失参考点的填补方法、参考点的滤波策略和模式信息的编码方案等重要细节\upcite{AdaptiveModeDecisionb}，但与将要提出的优化算法并无直接关系，暂不详述。在充分理解 H.26X 帧内预测的物理意义及预测过程后，下一节将分析其存在的缺陷并提出改进算法。
...
\label{cha:L-BPSec}
H.26X 的帧内分块方案中，基于四叉树结构的分块算法决定了一个预测单元仅可在保留 2N$\times$2N 结构与划分为 4 个 N$\times$N 结构这 2 个选项中进行选择。尽管这能有效地做到在不同纹理丰富程度的区域自适应选择分块大小，但过于粗暴的 2 选 1 导致每划分一次 N$\times$N 结构都产生 4 组需要独立编码的模式信息、分块信息及其他辅助信息，一定程度上降低了编码效率\upcite{SplitandMergeBasedBlockPartitioning}。本节将针对该缺陷进行分析与优化。

\subsection{编码块划分决策过程分析}
...
\subsection{帧内预测残差分析}
无损编码方案中跳过了量化步骤，此时待编码数据不再是变换域系数而是预测残差，因此其仍具有空间域的物理意义。为了直观地验证这一结论，尝试将残差绘制出来观察。图 \ref{fig:ResidualImage} 展示了在H.265标准下经过帧内预测后的残差图像（数值整体平移了128以绘制负值）\upcite{1DTransformMotionResidual,H264TwoLayerLosslessCoding}。
\begin{figure}[hbt]
    \centering
...
此外，为了证明 R-MED 算法性能良好，在表 \ref{tab:R-MEDOthers} 中给出了近年多种针对 H.265 无损帧内编码的优化算法与 R-MED 算法的性能比较。表中数据指对比 H.265 跳过变换量化方案的优化程度。由于大部分文献并未给出完整的测试序列的实验数据，因此仅能对比下表中所示的 5 个测试序列的性能：
\begin{table}[hbt]
\centering
...
\multirow{2}{*}{类别} &
  \multicolumn{1}{c}{\multirow{2}{*}{序列}} &
  \multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}SAP-\\ HV/\%\end{tabular}} &
  \multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}SAP\\ /\%\end{tabular}} &
  \multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}SAP1\\ /\%\end{tabular}} &
  \multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}SAP+SWP2\\ +DTM/\%\end{tabular}} &
  \multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}RDPCM\\ +pwm/\%\end{tabular}} &
  \multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}R-MED\\ /\%\end{tabular}} \\
...
\multicolumn{2}{l}{平均码率优化} & -4.94 & -7.47  & -7.57  & -8.67  & -6.99  & -8.29  \\ \midrule
\multicolumn{2}{l}{编码时间}   & 97    & 97     & 97     & 273    & 104    & 104    \\ \midrule
\multicolumn{2}{l}{解码时间}   & 99    & 99     & 98     & 196    & 103    & 95     \\ \bottomrule
\end{tabular}
\end{table}
上述算法中，SAP+SWP2+DTM\upcite{SAP-SAPHVSWP2DTM} 方案获得了最佳的码率优化，但编解码时间大幅增加。相比之下R-MED 以增加少量编码时间的代价获得了可观的码率优化，证明所提算法具有较高的实用价值。
