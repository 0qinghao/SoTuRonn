...
\begin{equation}
    X(k) = \sqrt{\frac{2}{N}}\varepsilon_{k}\sum_{n=0}^{N-1}x(n)\cos\left[\frac{k(2n+1)\pi}{2N}\right], \qquad k=0,1...,N-1
    \label{1ddct}
\end{equation}
...
\[
    ...
    \begin{cases}
        ...
        \sqrt{\frac{2}{N}}, \qquad o.w
    \end{cases}
\]

变换编码技术在 H.26X 中存在较多研究和优化内容不再赘述，但以下几点值得一提：
\begin{enumerate}
    \item DCT/DST 去相关性的性能随着变换大小的增大而增强，但增强的比率迅速减缓。因此考虑到计算复杂度和编码性能的折衷，较常采用的是 4x4 与 8x8 DCT，很少用到 16x16 及更大的变换；
    \item 
\end{enumerate}
