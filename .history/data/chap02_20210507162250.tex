...
具体的扫描顺序又分为水平扫描、垂直扫描和对角扫描，制定该 3 类扫描顺序的根据是预测模式与系数分布特征间的联系：一个采用（接近）垂直预测模式得到的待编码块，其能量很可能集中在前几行，因此水平扫描对（接近）垂直预测模式的块有利，同理垂直扫描对水平预测模式的块有利，对此 \ref{cha:IntraPredDetail} 节将做进一步分析。图 \ref{fig:ScanOrder} 给出了具体的 3 类扫描顺序。
\begin{figure}[hbt]
    \centering
