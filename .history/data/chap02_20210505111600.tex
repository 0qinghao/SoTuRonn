...
\begin{itemize}
    ...
    CTU 是 H.265 中基本的编码单元，CTU 一般会往下切分为多个 CU，CU 可切分为多个使用不同预测策略的 PU，PU 可选择适合自身的预测模式。由此 CTU 的率-失真优化分为 CU/PU 切分过程和 PU 的模式选择过程。类似地为了降低复杂度，当前 CU 的优化结束后不再考虑其对后续 CU 的影响。
\end{itemize}
