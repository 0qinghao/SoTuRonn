...
对不同的输入符号使用不同长度的码字进行编码称为变长编码。为了得到更高的编码效率，需要将短的码字分配给出现频率高的符号，反之将长码字分配给几乎不出现的符号以使平均码字长度最短。值得一提的是，变长编码中各码字必须符合“任意一个码字的尾缀不能是其他任意码字的前缀”的要求，否则变长码将不可译（在不添加码字间分隔符的情况下），符合该要求的一组码字一定是“唯一可译码”。
