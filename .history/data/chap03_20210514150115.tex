...
\section{本章小结}
本章从软件算法层面介绍了课题提出的优化算法。对 H.26X 无损编码中帧内预测、编码块划分过程进行分析，研究其存在的缺陷。针对缺陷提出了 L-IP、L-BP、R-MED 以及 L-BPIP 共 4 个优化算法，并分别进行了测试分析，编码效率最大提高超过 10\%，证明了算法的有效性。下一章介绍算法的硬件实现及 FPGA 原型验证系统。