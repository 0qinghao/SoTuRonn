...
此外，如果 R-MED 没有检测到明显的水平或垂直边缘，将采取模仿水平/垂直方向相邻点变化趋势的预测策略。如图 \ref{fig:RMEDcalculate}(c) 所示，$P_{i,j}^{'}$ 周围的参考点并不满足式 \ref{equ:LOCO-i} 的前两个条件，表明该区域变化平坦没有复杂纹理。$P_{i,j}^{'}$ 上方参考点的数值由 20 降低到 18(-2)，左侧参考点的数值由20增加到21（+1），因此$P_{i,j}^{'}$模仿上方的变化趋势得到$P_{i,j}^{'}=R_{i,j-1}-2=21-2=19$，或模仿左侧的变化趋势得到$P_{i,j}^{'}=R_{i-1,j}+1=18+1=19$。
