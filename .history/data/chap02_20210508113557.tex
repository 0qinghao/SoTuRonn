...
边界补偿采用对每个像素点单独进行分类和补偿的策略，出于性能与复杂度的折衷，分类过程仅涉及待处理像素及其相邻的 2 个像素。H.265 中需要通过 SAO 进行补偿的像素点共有 4 类，如图 \ref{fig:Gibbs} 与 \ref{fig:SAOCategories} 所示，图中 $c$ 表示当前待处理像素，$a,b$ 为其相邻的 2 个像素。
